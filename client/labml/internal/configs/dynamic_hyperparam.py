...
class DynamicHyperParam:
    r"""
    Dynamic hyperparameters can be changed manually on `labml.ai app <https://labml.ai>`
    while the models are training.
    There are designed as an alternative to hyperparameter schedules like gradually decreasing
    learning rate as the training progresses. Dynamic hyperparameters let users change
    the values based on training metrics instead of sticking to a pre-determined schedule.

    This is the base class that is extended by specific dynamic hyperparameter types.
...
class FloatDynamicHyperParam(DynamicHyperParam):
    r"""
    Use this for floating-point values.

    Arguments:
        default (float): default value of the hyperparameter

        range_ (Tuple[float, float]): the value range for the hyperparameter
    """
...
class IntDynamicHyperParam(DynamicHyperParam):
    r"""
    Use this for integer values.

    Arguments:
        default (int): default value of the hyperparameter

        range_ (Tuple[int, int]): the value range for the hyperparameter
    """
